...
> **Release-qualification status (2026-07-15, after real CI run):** CI (`.github/workflows/ci.yml`) executed on the pushed commit **acaae92** and **FAILED** — read in the browser at run [29384411393](https://github.com/Hassan-elbaron/helbaron-lms/actions/runs/29384411393). Product/UX defects remain fixed (CSP-01, D1–D3, LB-01…05, OBS-01/02, A11Y-EVENTS-01, A11Y-AUTH-01, SEC-01). **But two CI-blocking repository defects were introduced/surfaced:**
> - **ARCH-01 (High, introduced by DATA-01 fix — now REVERTED):** the DATA-01 seeder fix made `Catalog\...\CatalogSeeder` depend on `Authoring\Models\Section`/`Lesson` — a **bounded-context violation** that failed the API architecture test (`NoCrossContextEloquentAccessRule`) and Deptrac ("Catalog on Authoring"). **Fix:** reverted `CatalogSeeder` to its architecture-compliant original (no Authoring imports) and removed the added test. **DATA-01 is therefore REOPENED** (Low, data-quality) — its proper fix must seed curriculum from an architecture-exempt seeder (e.g. app-level `DatabaseSeeder`/`DemoSeeder`), not from the Catalog context.
> - **CI-01 (Medium, pre-existing — FIXED):** ESLint `react-hooks/rules-of-hooks` — `useState` called inside a Storybook `render` arrow in `pagination.stories.tsx` and `data-grid.stories.tsx`, failing the **Web** lint job (and, since `next build` runs ESLint, the **E2E** web-server start). **Fix:** extracted each hook into a proper component (`InteractivePagination`, `PaginatedGrid`).
>
> **CI re-run #6 (commit 44b8537) result:** ARCH-01 + CI-01 **confirmed fixed** — **API, Architecture (Deptrac), and Secret scan are now GREEN**. Still red: **Web** (`tsc` — Storybook type errors), **E2E** (cascade of the Web build), and **API image** (**CI-02**: `trivy-action@0.28.0` version doesn't exist).
>
> **Fixes staged for the next push (pending host `npm run typecheck` + `git push`):**
> - **CI-02 (CI-config, FIXED):** both Trivy steps in `.github/workflows/ci.yml` (API image + Web image) now pin the action to the **immutable commit SHA** `aquasecurity/trivy-action@ed142fd0673e97e23eac54620cfb913e5ce36c25 # v0.36.0` — not a floating `@master`, per policy. All scan settings unchanged (`format: table`, `exit-code: '1'`, `severity: CRITICAL,HIGH`, `ignore-unfixed: true`); scanning is **not** disabled or weakened.
> - **CI-03 (Web typecheck, Storybook story types — FIXED, no suppression):** the failing `tsc --noEmit` errors were CSF3 stories whose required component props were unsatisfied. Fixed by supplying the real required props (no `any`, `@ts-ignore`, `@ts-expect-error`, unsafe casts, tsconfig weakening, or Storybook exclusions). Files: `commerce/product-card.stories.tsx` (`onAdd`), `marketing/course-preview-card.stories.tsx` (`onPlay`), `reports/report-view.stories.tsx` (`payload`/`meta`/`page`/`onPageChange`), `ui/pagination.stories.tsx` (`onPageChange` on the Interactive story), `student/query-state.stories.tsx` (`query`/`children` meta defaults + `new globalThis.Error` to avoid the `export const Error` shadow → TS2351), `ui/confirm-dialog.stories.tsx` (`open`/`onOpenChange`/`onConfirm`), `ui/form-field.stories.tsx` (`label`/`children`), `ui/popover.stories.tsx` (`children`), `ui/tooltip.stories.tsx` (`children`). Interactive stories keep their `render`-owned state (hooks already extracted into named components under CI-01); the meta defaults only make render-only stories type-safe. Storybook interaction behavior preserved.
>
> **Decision: NO GO (pending re-run).** These fixes are staged but the release gate is the **live CI run**, which cannot be triggered from this QA env (no push access) and cannot be trustworthy-verified locally (sandbox mount serves truncated file views, so sandbox `tsc` is unusable). GO/NO-GO will be re-decided only after the host `npm run typecheck` passes, the commit is pushed, and the resulting CI run is read job-by-job. See FINAL_RELEASE_READINESS_REPORT.md → "CI Run #6 → staged fixes".
...
### DATA-01 — [Low, demo-data quality] Seeded courses Published with 0 sections/lessons (violates the publish invariant) — **REOPENED**
**Attempted fix REVERTED (2026-07-15):** the fix added `seedMinimalCurriculum()` to `CatalogSeeder`, calling `Authoring\Models\Section`/`Lesson` directly. Real CI proved this is a **bounded-context architecture violation** (Catalog → Authoring is forbidden; failed the API architecture test + Deptrac). It has been **reverted** (see ARCH-01 in the banner). DATA-01 is a **Low data-quality** item: the base `CatalogSeeder` courses are Published without curriculum. A **compliant** fix must attach curriculum from an architecture-exempt seeder (app-level `DatabaseSeeder`/`DemoSeeder`, which may legitimately touch both contexts) — not from the Catalog domain seeder. Deferred, non-blocking. Original finding retained below.
